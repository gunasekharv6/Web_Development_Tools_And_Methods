    ...
    <body>
        ...
        <form action="cart.htm" method="post">
            ...
            <div id="book">
                <c:forEach items="${requestScope.list}" var="item" varStatus="loopCounter">
                    <input type="checkbox" name="chkItem" id="book${loopCounter.index}" value="${item.title}">
                    <label for="book${loopCounter.index}"> ${item.title} [${item.price}]</label>
                    <br>
                </c:forEach>
            </div>
            <br/><br/>
        ...
        <script>
            function confirm_update() {
                var cont = document.getElementById('book').children;
                var count = 0;
